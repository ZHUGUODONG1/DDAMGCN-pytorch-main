...
class Decoder(nn.Module):
    def __init__(self, device, k_num,num_nodes, l, length=12, residual_channels=32, dilation_channels=32, K=3, Kt=3):
...
class Encoder(nn.Module):
    def __init__(self, device, k_num,num_nodes, length=12,
...
class DDAMGCN(nn.Module):
    def __init__(self, device, k_num, num_nodes, l, dropout=0.5, supports=None, length=12,
        ...
        self.encoder=Encoder(num_nodes, length=length,
                 in_dim=in_dim,  residual_channels=residual_channels, dilation_channels=dilation_channels, K=K, Kt=Kt)
        self.decoder = Decoder(num_nodes, length=length,
    ...
    def forward(self, input):
        ...
        v=self.encoder(x,Trend_Matrix,A)
