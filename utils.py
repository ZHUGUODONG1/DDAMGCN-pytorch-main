...
class MSTGCN(nn.Module):
    def __init__(self, c_in, c_out, num_nodes, tem_size, K, Kt):
        ...
        self.conv4_9 = Conv2d(c_in, c_out, kernel_size=(1, 9), padding=(0, 4),
                              stride=(1, 1), bias=True)
        self.gcn = DGCN(c_out, 2 * c_out, K, 1)
        self.tgcn=Trend_GCN(c_out, c_out,K)
    ...
    def forward(self, x,Trend_Matrix, supports):
        ...
        z1=self.tgcn(x_m,Trend_Matrix)
        z2 = self.gcn(x_m, supports)
        x=self.gf(z1,z2,x_input1)
        return x


class Key_Node_Identification_Layer(nn.Module):
    def __init__(self, K=10, input_dim=32):
        super(Key_Node_Identification_Layer, self).__init__()
        self.K = K
        self.FC1 = nn.Conv2d(1, input_dim, kernel_size=(1, 1),
                                 stride=(1, 1), bias=True)
        self.FC2 = nn.Conv2d(1, input_dim, kernel_size=(1, 1),
                                 stride=(1, 1), bias=True)
...
class Dynamic_Delay_Aware_Module(nn.Module):
    ...
    def __init__(self, K=10,input_dim=32,length=12):
        super(Dynamic_Delay_Aware_Module, self).__init__()
        self.K = K
        # self.linear_layer = nn.Linear(K, 1)
        self.linear_layer = nn.Conv2d(K, 1, kernel_size=1)
        self.time_pro = nn.Conv3d(in_channels=length, out_channels=1, kernel_size=(1, 1, 1), stride=(1, 1, 1))
        self.time_linear_layer = nn.Linear(length, 1)
        self.KNIL=Key_Node_Identification_Layer(K,input_dim)
        self.weights = nn.Parameter(torch.randn(length))
        self.scale=torch.arange(1, length+1, dtype=torch.float32).view(1,length , 1, 1, 1)
    ...
    def forward(self, data):
        ...
        selected_values,_=self.KNIL(input_data)
        t = 0
        delay_all_time=[]
        while t < length:
